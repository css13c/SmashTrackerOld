defmodule SmashTracker.Skill do
  ...
  def update_rating(players) do
    # Putting in a safeguard here since this is only tested and verified for 2 players
    if length(players) <= 2 do
      ...
      Enum.map(f_players, fn {id, i_mu, i_sig, i} ->
        i_sigsq = i_sig * i_sig
        {i_omega, i_delta } = f_players
          |> Enum.filter(fn {_, _, _, q} -> i != q end)
          |> Enum.reduce({0, 0}, fn {_, q_mu, q_sig, q}, {omega, delta} ->
            q_sigsq = q_sig * q_sig
            ...
            {omega + sigsq_to_ciq * (s - piq), delta + gamma * sigsq_to_ciq / ciq * piq * (1 - piq)}
          end)

        {id, i_mu + i_omega, i_sig * Math.sqrt(max(1 - i_delta, 0.0001))}
      end)
    else
      throw "Cannot update ratings for a game that has more than 2 players"
    end
  end
